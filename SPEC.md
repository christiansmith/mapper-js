...
| **Name string**               | `"mapping:Person"`                   | A registry reference: the named mapping is substituted (§3.5). A string that is none of the above — bare URI-fragment strings included (§4.2) — is invalid: it evaluates to undefined and raises a diagnostic naming the string **[DOC-2]**. |
...
a JSON string pointer. The fragment form is valid only as the value of a
locate keyword, not as a bare string descriptor (§3.3). In YAML serializations
it must be quoted: an unquoted `#` begins a comment.
...
- **DEREF(d)** — resolves references: a string that names a registered
  ...
  (§3.5); everything else passes through. A string that no branch of GET
  recognizes is an unrecognized descriptor: it evaluates to undefined and
  raises a diagnostic (§3.3, DOC-2).
...
- **Registry operations.** `$extend` resolution is performed at registration
  (§3.5), including evaluation-time registration: evaluation-time
...
| REG-3       | §3.5    | `13-audit-probes` (A11)                                         | A11       |
| REG-4       | §3.5    | `13-audit-probes` (A11)                                         | A11       |
