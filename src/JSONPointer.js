 ...
 */
class JSONPointer {
  static get(source, expr) {
    const pointer = new JSONPointer(expr, 1)
    return pointer.get(source)
  }

  static set(target, expr, value) {
    const pointer = new JSONPointer(expr, 1)
  ...
  parseJSONString(expr) {
    ...
    if (expr.charAt(0) !== '/') {
      throw new Error(`Invalid JSON Pointer ${expr}`)
    }
